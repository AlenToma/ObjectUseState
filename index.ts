...
const addToWait = async <T>(value: any, keyValue: any, key: string, setValue: Function, parent: UseState<T>) => {
  parent.watingList.push(async () => await set(value, keyValue, key, setValue, parent));
  if (parent.isWorking)
    return;
...
const set = async <T>(value: any, keyValue: any, key: string, setValue: Function, parent: UseState<T>) => {
  try {
...
class UseState<T> {
  ...
  constructor(item: T, hierarkiUseState?: boolean) {
    this.watingList = [];
  ...
  public setValue = (item: T) => {
    ...
    Object.keys(item).forEach((x) => {
      var v = (item as any)[x];
      if (this.hierarkiUseState && v && typeof v === 'object' && v !== null && !Array.isArray(v))
...
export default <T>(item: T, hierarkiUseState?: boolean) => {
  const state = new UseState<T>(item, hierarkiUseState) as T | UseState<T>;
  return state as T;
};
